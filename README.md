...
tables and bout counts above are computed rather than typed. Run
`make readme` to regenerate `README.md`, which is what GitHub shows.
That needs pandoc; the container has it already. Editing `README.md` is
pointless, since the next render overwrites it.
...
the vignettes now live in `docs/` as ordinary documents.
