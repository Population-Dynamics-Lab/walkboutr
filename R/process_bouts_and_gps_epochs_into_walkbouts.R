...
#' parameters and constants to determine whether an epoch is active or inactive, the
#' minimum number of epochs for a period of activity to be considered as a potential
#' bout, the local time zone of the data, and other relevant information.
...
#'
#' @details The function first collates the arguments passed to it with the collate_arguments()
#'   function. It then merges "gps_epochs" and "bouts" data frames by "time" column, and orders
#'   the resulting data frame by "time". Then, it generates the "bout_radius" using the
#'   generate_bout_radius() function, which calculates the radius of a bounding circle that would
#'   be considered a dwell bout. Next, the function evaluates the completeness of GPS data using
#'   the evaluate_gps_completeness() function, which determines the number of GPS observations
#'   within a bout and the ratio of data points with versus without GPS data. Finally, the
#'   function generates the "bout_category" using the generate_bout_category() function, which
#'   determines whether a bout is a walk bout or a dwell bout, and calculates the complete days,
#'   non-wearing periods, and speed.
...
#' median walking speed is calculated for only the latest available GPS data point in each
#' epoch.
#'
...
#' This function identifies outlier GPS points for the bout radius calculation from a given set
#' of latitude and longitude coordinates.
#'
#' @param lat_long A data frame containing the latitude and longitude coordinates for the GPS
#'   points.
#' @param dwellbout_radii_quantile The threshold for outliering GPS data points - any data
#'   points above the specified percentile are outliered.
#'
...
#'
#' This function generates a bounding circle radius for each walking bout identified in the
#' input data. The bounding circle is defined as the smallest circle that fully contains all
#' GPS locations observed during a walking bout.
#'
#' @param walk_bouts A data frame containing GPS locations for each walking bout, with columns
#'   "longitude", "latitude", and "bout" (a unique identifier for each bout)
#' @param dwellbout_radii_quantile A quantile (between 0 and 1) used to filter outlying GPS
#'   data points before generating the bounding circle. GPS points with a distance from the
#'   center greater than the radius of the circle that contains (1 - dwellbout_radii_quantile) of
#'   the GPS points are considered outliers and are excluded.
#'
...
generate_bout_radius <- function(walk_bouts, dwellbout_radii_quantile){
  ...
  # pull long/lat and remove outliers
    ...
    bout_radii <- base::rbind(bout_radii, data.frame(bout = bout_label, bout_radius = circle_radius))
  }
  return(bout_radii)
}
...
#'
#' This function evaluates the completeness of GPS data for each walking bout. For each bout,
#' it checks if the number of valid GPS records (with speed, latitude, and longitude data) is
#' greater than a specified threshold, and if the ratio of valid GPS records to total records
#' is greater than a specified minimum. If both of these conditions are met, the function
#' considers the GPS data for the bout to be complete. The function also calculates the median
#' speed for each bout.
#'
#' @param walk_bouts A data frame containing information about walking bouts, including GPS
#'   data.
#' @param min_gps_obs_within_bout The minimum number of GPS observations required for a bout to
#'   be considered to have complete GPS data.
#' @param min_gps_coverage_ratio The minimum ratio of GPS observations with valid data to total
#'   GPS observations for a bout to be considered to have complete GPS data.
#'
...
#'
#' Given accelerometer bout data, this function generates bout categories, which includes dwell
#' bouts, non-walk bouts that are either too slow, too fast, or too vigorous, and bouts with an
#' unknown lack of GPS data.
#'
...
#' @param max_dwellbout_radii_ft a numeric scalar that specifies the maximum radius, in feet,
#'   of a bounding circle that would be considered a dwell bout.
#' @param max_walking_cpe a numeric scalar that specifies the maximum activity counts per epoch
#'   value before the accelerometer is considered to be picking up on an activity other than
#'   walking.
#' @param min_walking_speed_km_h a numeric scalar that specifies the minimum speed considered
#'   walking.
#' @param max_walking_speed_km_h a numeric scalar that specifies the maximum speed considered
#'   walking.
#'
#' @returns The walk bout data frame with a single `bout_category` column added, holding
#'   one of `walk_bout`, `dwell_bout`, `non_walk_too_vigorous`, `non_walk_slow`,
#'   `non_walk_fast` or `non_walk_incomplete_gps`.
#'
#' @details The function uses the bout information for walking bouts, bout radii information,
#'   and GPS data completeness information to generate the bout categories.
#'
#' The function first generates dwell bouts by joining the bout radii information and GPS data
#' completeness information on the bout column, and then filters out the rows that have bout
#' values that are missing using the filter function. Then, it calculates the dwell bout values
#' as TRUE if the complete_gps column is TRUE and the bout_radius column is less than
#' max_dwellbout_radii_ft. The resulting data frame only contains the bout and dwell_bout
#' columns.
#' The function then joins the resulting data frame with the walking bout data frame using the
#' bout column. Then, for the non-walk bouts, the function calculates whether they are too
#' vigorous, too slow, or too fast. For the non-walk bouts that are too vigorous, the function
#' calculates the mean activity_counts for each bout, and then sets the non_walk_too_vigorous
#' value as TRUE if the mean activity_counts value is greater than max_walking_cpe. For the
#' non-walk bouts that are too slow or too fast, the function calculates the median speed for
#' each bout, and then sets the non_walk_slow or non_walk_fast value as TRUE if the median
#' speed value is less than min_walking_speed_km_h or greater than max_walking_speed_km_h,
#' respectively. Finally, the function generates a non_walk_incomplete_gps value as TRUE if the
#' complete_gps value is FALSE for the bout.
#' Each bout starts as a walk bout and each rule below overwrites the previous one, so
#' the last rule to match wins. In precedence order, highest first:
