...
```{r pipeline-map, echo = FALSE, results = "asis"}
# R/README.md is the single source for this. It is authored there so that GitHub
# renders it when someone browses into the R/ folder, and pulled in here so the
# same explanation is on the landing page. Headings are demoted one level to sit
# under this document's structure.
lines <- readLines("R/README.md", warn = FALSE)
in_code <- FALSE
for (i in seq_along(lines)) {
  if (grepl("^```", lines[i])) in_code <- !in_code
  if (!in_code) lines[i] <- sub("^(#+) ", "\\1# ", lines[i])
}
cat(lines, sep = "\n")
```
